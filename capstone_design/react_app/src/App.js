...
function App() {
    ...
    const handleRefresh = async () => {
        await fetch(`${process.env.REACT_APP_API_BASE_URL}:5000/api/reset_session`, {
            method: 'GET',
        });
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (inputMessage.includes("직업을 추천해")) {
                ...
                let response = await fetch(`${process.env.REACT_APP_API_BASE_URL}:5000/api/predict`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    response = await fetch(`${process.env.REACT_APP_API_BASE_URL}:5000/api/get_job_info`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                ...
                if (jobList) { // 직업 추천을 받았다면
                    ...
                    if (jobIndex >= 0 && jobIndex < jobKeys.length) {
                        ...
                        response = await fetch(`${process.env.REACT_APP_API_BASE_URL}:5000/api/get_job_detail/${jobInfo}`, {
                            method: 'GET',
                        });
                    } else {
                ...
                response = await fetch(`${process.env.REACT_APP_API_BASE_URL}:5000/api/chat`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
